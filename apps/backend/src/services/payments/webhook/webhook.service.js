        ...
        async getWebhookStats() {
            ...
            const [total, processed, failed, stripeCount, razorpayCount, chargebackCreated, chargebackClosed] = await Promise.all([
                ...
                this.paymentEventRepo.count({
                    where: {
                        isProcessed: false,
                        createdAt: (0, typeorm_1.MoreThanOrEqual)(twentyFourHoursAgo)
                    }
                }),
                this.webhookRepo.count({ where: { gateway: 'stripe' } }),
                this.webhookRepo.count({ where: { gateway: 'razorpay' } }),
                this.paymentEventRepo.count({ where: { event: 'chargeback_received', createdAt: (0, typeorm_1.MoreThanOrEqual)(twentyFourHoursAgo) } }),
                this.paymentEventRepo.count({ where: { event: 'chargeback_closed', createdAt: (0, typeorm_1.MoreThanOrEqual)(twentyFourHoursAgo) } }),
            ]);
            return {
