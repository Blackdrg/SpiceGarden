...
export class LegalSeedService implements OnModuleInit {
  ...
  private documentSpecs(): {
    ...
    return [
      ...
      {
        type: LegalDocumentType.DELIVERY_SOP,
        title: 'Delivery Standard Operating Procedure',
        summary: 'Step-by-step operational procedures for delivery partners and operations teams.',
        requiresAcceptance: true,
        sections: [
          this.section('pre_shift', 'Pre-Shift Checks', 'Delivery partners must verify vehicle condition, phone battery, app version, and active orders before starting a shift. GPS must be enabled and accurate.', 1),
          this.section('order_acceptance', 'Order Acceptance', 'Accept orders within the assigned radius. Verify restaurant name, order items, and pickup code before marking as picked up. Do not accept orders outside operating hours or for prohibited items.', 2),
          this.section('pickup', 'Pickup Procedure', 'Arrive at the restaurant within the estimated pickup window. Collect all items, verify order completeness against the app, and confirm pickup via the app. Report missing or incorrect items to support immediately.', 3),
          this.section('transit', 'In-Transit Protocol', 'Follow the optimized route provided by the app. Maintain safe driving standards. Do not deviate from the route without app direction. Keep the customer informed of significant delays via the app.', 4),
          this.section('delivery', 'Delivery Procedure', 'Verify customer identity when required. Hand over the order in good condition. Obtain customer signature or OTP confirmation in the app. For cash-on-delivery, collect the exact amount and confirm via the app.', 5),
          this.section('contactless', 'Contactless Delivery', 'Where selected, leave the order at the specified location, take a clear photo, and upload it through the app. Do not enter the customer\'s premises unless explicitly invited.', 6),
          this.section('issue_handling', 'Issue Handling', 'If an order is damaged, spilled, or incorrect, do not deliver. Contact support and the restaurant through the app. Do not attempt to fix the order yourself. Document the issue with photos.', 7),
          this.section('end_of_shift', 'End of Shift', 'Return any un-delivered items per the cancellation policy. Submit daily cash reconciliation if handling COD. Report vehicle or app issues through the partner dashboard.', 8),
        ],
      },
      {
        type: LegalDocumentType.ESCALATION_SOP,
        title: 'Escalation Standard Operating Procedure',
        summary: 'Tiered escalation matrix for customer complaints, delivery incidents, and operational emergencies.',
        requiresAcceptance: false,
        sections: [
          this.section('overview', 'Overview', 'This SOP defines escalation paths for incidents ranging from delayed deliveries to safety emergencies. All personnel must follow the prescribed tiers and SLAs.', 1),
          this.section('tier1', 'Tier 1 — Frontline Resolution', 'First contact: support agent or delivery partner resolves within 15 minutes. Scope: missing items, minor delays, wrong address, payment discrepancies. Action: acknowledge, investigate, resolve or offer compensation within authority limits.', 2),
          this.section('tier2', 'Tier 2 — Supervisor Escalation', 'Escalate when: Tier 1 cannot resolve within 15 minutes; customer explicitly requests a supervisor; issue involves food safety, harassment, or vehicle accident; financial exposure exceeds ₹500. Action: supervisor reviews case, contacts customer within 30 minutes, and issues resolution or escalates to Tier 3.', 3),
          this.section('tier3', 'Tier 3 — Management Escalation', 'Escalate when: Tier 2 cannot resolve within 30 minutes; issue involves serious injury, police involvement, or media risk; financial exposure exceeds ₹5,000; systemic failure affects multiple customers. Action: ops manager engages, coordinates with legal/compliance, and communicates status to affected parties within 1 hour.', 4),
          this.section('tier4', 'Tier 4 — Executive and External Escalation', 'Escalate when: regulatory inquiry or legal notice; data breach; mass order failure (>50 orders); customer fatality or serious injury. Action: CTO/COO notified within 15 minutes. External stakeholders (law enforcement, regulators, insurers) contacted per legal counsel guidance. PR hold until approved.', 5),
          this.section('sos', 'SOS / Emergency Protocol', 'For immediate danger: customer or partner triggers SOS in-app. System automatically alerts operations center, nearest available delivery partners within 5 km, and emergency contacts on file. If police or ambulance is needed, ops center dials 100/108 immediately and logs the incident.', 6),
          this.section('communication', 'Communication Standards', 'All escalations must be logged in the ticketing system with severity, affected parties, actions taken, and resolution timeline. No promise of compensation beyond authority limits without manager approval. Keep the customer informed every 30 minutes during active escalation.', 7),
          this.section('post_incident', 'Post-Incident Review', 'Within 24 hours of Tier 3/4 resolution, conduct a blameless post-mortem. Document root cause, remediation steps, and process improvements. Update this SOP if a new failure mode is identified.', 8),
        ],
      },
      {
