...
let ChargebackService = ChargebackService_1 = class ChargebackService {
    ...
    async initiateRefundForWonDispute(disputeId, processedBy, gateway) {
        const dispute = await this.disputeRepo.findOne({
            where: { disputeId },
            relations: { order: true }
        });
        if (!dispute) {
            throw new common_1.NotFoundException(`Chargeback dispute ${disputeId} not found`);
        }
        if (dispute.status !== 'won') {
            throw new common_1.BadRequestException(`Cannot initiate refund for dispute with status: ${dispute.status}. Only won disputes are eligible for refund.`);
        }
        ...
        if (dispute.order) {
            await this.productionNotification.sendPaymentNotification(dispute.order.userId, `refund-${disputeId}`, {
