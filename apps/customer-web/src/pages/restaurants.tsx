...
import { Button, Card, DESIGN_TOKENS, SkeletonCard, HomeIcon, SearchIcon as SearchIconLucide, CartIcon, ProfileIcon } from '@spicegarden/ui';
import { useRouter } from 'next/router';
import { useOfflineQueue } from '../hooks/useOfflineQueue';
import { Star, WifiOffIcon } from 'lucide-react';
...
function restaurantsReducer(state: RestaurantsState, action: { type: string; payload?: unknown }): RestaurantsState {
  switch (action.type) {
    case 'SET_QUERY':
...
const RestaurantsPage = () => {
  ...
  const loadRestaurants = useCallback(async () => {
    ...
    try {
      ...
      const list = Array.isArray(data) ? data : (data as any)?.restaurants ?? [];
      dispatch({ type: 'SET_RESTAURANTS', payload: list });
    } catch (error) {
      console.error('Failed to load restaurants:', error);
      dispatch({ type: 'SET_ERROR', payload: 'Failed to load restaurants. Please check your connection.' });
  ...
  useEffect(() => {
    const debounceTimer = setTimeout(loadRestaurants, 300);
    return () => clearTimeout(debounceTimer);
  }, [loadRestaurants]);

  const filtered = state.restaurants.filter((r) => {
    if (state.activeFilter === 'all') return true;
    if (state.activeFilter === 'rated 4+') return (r.rating ?? 0) >= 4;
    return true;
  });
  ...
  return (
    <div className={styles.pageContainer}>
      <div className={styles.pageHeader}>
        <h2 className={styles.pageTitle}>Restaurants</h2>
      </div>
      ...
      {state.error ? (
        ...
        <div className={styles.emptyState}>
          ...
          <p className={styles.emptyText}>Try a different search or check back later.</p>
          <Button label="Go Home" onClick={() => router.push('/')} variant="outline" />
        </div>
      ) : (
      ...
      <nav className={styles.bottomNav}>
        {[
          ...
          <button
            ...
            className={`${styles.navButton} ${tab.key === 'home' ? styles.navActive : styles.navInactive}`}
