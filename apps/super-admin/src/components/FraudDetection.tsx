      ...
      {tickets.map((ticket) => (
        <div key={ticket.id} style={{ padding: '12px', background: '#fff5f5', borderRadius: 8, marginBottom: 8, borderLeft: '4px solid #f04e31' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <AdminButton label="Investigate" onClick={() => alert(`Opening case ${ticket.id}`)} style={{ flex: 1, padding: '6px 0', fontSize: 12 }} />
            <AdminButton label="Block IP" onClick={() => alert(`IP blocked for ${ticket.id}`)} variant="secondary" style={{ flex: 1, padding: '6px 0', fontSize: 12 }} />
          </div>
        </div>
      ))}
    </DashboardCard>
