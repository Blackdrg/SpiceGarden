...
exports.StoreManager = void ;
const electron_ = require("electron");
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const electron_store_ = __importDefault(require("electron-store"));
class StoreManager {
    store;
    secretsPath;
    constructor() {
        this.store = new electron_store_.default({
        ...
        this.secretsPath = path.join(electron_.app.getAppPath(), '..', '..', 'secrets');
    }
    getConfig() {
    ...
    getSecrets() {
        try {
            ...
            for (const file of files) {
                ...
                const value = fs.readFileSync(path.join(this.secretsPath, file), 'utf-').trim();
                if (key) {
                    secrets[key] = value;
                }
            }
